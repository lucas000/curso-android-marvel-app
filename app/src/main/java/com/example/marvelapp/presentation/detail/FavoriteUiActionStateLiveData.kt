...
class FavoriteUiActionStateLiveData(
    ...
    private var currentFavoriteItem = R.drawable.ic_favorite_unchecked
            ...
            when(it) {
                is Action.CheckFavorite -> {
                    checkFavoriteUseCase.invoke(
                        ...
                        success = { isFavorite ->
                            if (isFavorite) {
                                currentFavoriteItem = R.drawable.ic_favorite_checked
                            }
                            emitFavoriteIcon()
                        },
                        error = {}
                ...
                is Action.RemoveFavorite -> {
                    it.detailViewArg.run {
                        removeFavoriteUseCase.invoke(
                            ...
                            success = {
                                currentFavoriteItem = R.drawable.ic_favorite_unchecked
                                emitFavoriteIcon()
                            },
                            error = {
    ...
    private suspend fun LiveDataScope<UiState>.emitFavoriteIcon() {
        emit(UiState.Icon(currentFavoriteItem))
    }
    ...
    fun update(detailViewArg: DetailViewArg) {
        action.value = if (currentFavoriteItem == R.drawable.ic_favorite_unchecked) {
